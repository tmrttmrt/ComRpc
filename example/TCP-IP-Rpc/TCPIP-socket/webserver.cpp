...
void webserver_root() {
	...
	if (server.method() == HTTP_GET) {
		...
		page_content += F("<b>Socket Settings");
		page_content += F("</b><br/>");
		page_content += F("<table>");
		page_content += form_input("socket_port", F("socket_port"), String(currSettings.socket_port), 30);
		page_content += F("</table><br/><hr/><b>");
...
#define readPasswdParam(param,dest) if (server.hasArg(#param)){ i = 0; masked_pwd = ""; for (i=0;i<server.arg(#param).length();i++) masked_pwd += "*"; if (masked_pwd != server.arg(#param) || server.arg(#param) == "") { server.arg(#param).toCharArray(dest, sizeof(dest)); } }
		...
		readIntParam(socket_port, currSettings.socket_port);
#undef readCharParam
#undef readBoolParam
#undef readIntParam
		...
		page_content += line_from_value(F("socket_port"), String(currSettings.socket_port));
		page_content += F("<br/>");
		page_content += F("<br/>");
...
void setup_webserver() {
	server_name  = F("TCPIP-socket-RPC-");
